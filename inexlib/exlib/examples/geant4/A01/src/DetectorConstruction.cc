...
namespace A01 {
class DetectorConstMessenger: public G4UImessenger {
public:
  DetectorConstMessenger(DetectorConstruction* mpga):target(mpga) {
...
A01::DetectorConstruction::DetectorConstruction()
 : air(0), argonGas(0), scintillator(0), CsI(0), lead(0),
   ...
   armAngle(30.*CLHEP::deg), secondArmPhys(0)

{
  messenger = new DetectorConstMessenger(this);
  magneticField = new A01::MagneticField();
...
G4VPhysicalVolume* A01::DetectorConstruction::Construct()
{
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  G4VSolid* worldSolid = new G4Box("worldBox",10.*CLHEP::m,3.*CLHEP::m,10.*CLHEP::m);
  ...
  G4VSolid* magneticSolid = new G4Tubs("magneticTubs",0.,1.*CLHEP::m,1.*CLHEP::m,0.,360.*CLHEP::deg);
  ...
  // first arm
  G4VSolid* firstArmSolid = new G4Box("firstArmBox",1.5*CLHEP::m,1.*CLHEP::m,3.*CLHEP::m);
  G4LogicalVolume* firstArmLogical
    = new G4LogicalVolume(firstArmSolid,air,"firstArmLogical",0,0,0);
  new G4PVPlacement(0,G4ThreeVector(0.,0.,-5.*CLHEP::m),firstArmLogical,
                    "firstArmPhysical",worldLogical,0,0);

  // second arm
  G4VSolid* secondArmSolid = new G4Box("secondArmBox",2.*CLHEP::m,2.*CLHEP::m,3.5*CLHEP::m);
  ...
  G4VSolid* hodoscope1Solid = new G4Box("hodoscope1Box",5.*CLHEP::cm,20.*CLHEP::cm,0.5*CLHEP::cm);
  ...
  for(int i1=0;i1<15;i1++)
  {
    ...
    new G4PVPlacement(0,G4ThreeVector(x1,0.,-1.5*CLHEP::m),hodoscope1Logical,
                      "hodoscope1Physical",firstArmLogical,0,i1);
  }

  // drift chambers in first arm
  G4VSolid* chamber1Solid = new G4Box("chamber1Box",1.*CLHEP::m,30.*CLHEP::cm,1.*CLHEP::cm);
  ...
  // "virtual" wire plane
  G4VSolid* wirePlane1Solid = new G4Box("wirePlane1Box",1.*CLHEP::m,30.*CLHEP::cm,0.1*CLHEP::mm);
  ...
  G4VSolid* hodoscope2Solid = new G4Box("hodoscope2Box",5.*CLHEP::cm,20.*CLHEP::cm,0.5*CLHEP::cm);
  ...
  G4VSolid* chamber2Solid = new G4Box("chamber2Box",1.5*CLHEP::m,30.*CLHEP::cm,1.*CLHEP::cm);
  ...
  // "virtual" wire plane
  G4VSolid* wirePlane2Solid = new G4Box("wirePlane2Box",1.5*CLHEP::m,30.*CLHEP::cm,0.1*CLHEP::mm);
  ...
  // CsI calorimeter
  G4VSolid* EMcalorimeterSolid = new G4Box("EMcalorimeterBox",1.5*CLHEP::m,30.*CLHEP::cm,15.*CLHEP::cm);
  G4LogicalVolume* EMcalorimeterLogical
    = new G4LogicalVolume(EMcalorimeterSolid,CsI,"EMcalorimeterLogical",0,0,0);
  new G4PVPlacement(0,G4ThreeVector(0.,0.,2.*CLHEP::m),EMcalorimeterLogical,
                    "EMcalorimeterPhysical",secondArmLogical,0,0);

  // EMcalorimeter cells
  G4VSolid* cellSolid = new G4Box("cellBox",7.5*CLHEP::cm,7.5*CLHEP::cm,15.*CLHEP::cm);
  G4LogicalVolume* cellLogical
    = new G4LogicalVolume(cellSolid,CsI,"cellLogical",0,0,0);
  G4VPVParameterisation* cellParam = new A01::CellParameterisation();
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  new G4PVReplica("HadCalColumnPhysical",HadCalColumnLogical,
		  HadCalorimeterLogical,kXAxis,10,30.*CLHEP::cm);

  // hadron calorimeter cell
  G4VSolid* HadCalCellSolid
    = new G4Box("HadCalCellBox",15.*CLHEP::cm,15.*CLHEP::cm,50.*CLHEP::cm);
  ...
  new G4PVReplica("HadCalCellPhysical",HadCalCellLogical,
		  HadCalColumnLogical,kYAxis,2,30.*CLHEP::cm);

  // hadron calorimeter layers
  G4VSolid* HadCalLayerSolid
    = new G4Box("HadCalLayerBox",15.*CLHEP::cm,15.*CLHEP::cm,2.5*CLHEP::cm);
  ...
  new G4PVReplica("HadCalLayerPhysical",HadCalLayerLogical,
                  HadCalCellLogical,kZAxis,20,5.*CLHEP::cm);

  // scintillator plates
  G4VSolid* HadCalScintiSolid
    = new G4Box("HadCalScintiBox",15.*CLHEP::cm,15.*CLHEP::cm,0.5*CLHEP::cm);
  G4LogicalVolume* HadCalScintiLogical
    = new G4LogicalVolume(HadCalScintiSolid,scintillator,"HadCalScintiLogical",0,0,0);
  new G4PVPlacement(0,G4ThreeVector(0.,0.,2.*CLHEP::cm),HadCalScintiLogical,
...
void A01::DetectorConstruction::ConstructMaterials()
{
  G4double a;
  G4double z;
  G4double density;
  ...
  density = 1.782e-03*CLHEP::g/CLHEP::cm3;
  ...
  // CsI
  density = 4.51*CLHEP::g/CLHEP::cm3;
...
void A01::DetectorConstruction::DestroyMaterials()
{
...
void A01::DetectorConstruction::DumpGeometricalTree(G4VPhysicalVolume* aVolume,G4int depth)
{
...
void A01::DetectorConstruction::SetArmAngle(G4double val)
{
  if(!secondArmPhys)
  {
...
/// messenger : ///////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
void A01::DetectorConstMessenger::SetNewValue(G4UIcommand * command,G4String newValue)
{
  if( command==armCmd )
  { target->SetArmAngle(armCmd->GetNewDoubleValue(newValue)); }
}

G4String A01::DetectorConstMessenger::GetCurrentValue(G4UIcommand * command)
{
  G4String cv;
  if( command==armCmd )
